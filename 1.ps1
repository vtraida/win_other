...
function GetMenu { 
    ...
    "" 
    "" 
    "1) PC Serial Number_BIOS version" 
    "2) PC Printer Info" 
    "3) Current User" 
    "4) OS Info" 
    "5) System Info" 
...
function GetInfo{ 
    ...
    switch ($MenuSelection){ 
        ...
        5 { #System Info 
            gwmi -computer $compname Win32_ComputerSystem | Format-List Name,Domain,Manufacturer,Model,SystemType,BIOSVersion 
            Pause 
            CheckHost          
          }         
        ...
        14 { #Monitor Info 
		 ...
		 15 {#OS+Processor+Memory Info
				"User" >> D:\PC_MEM_Proc.txt
				gwmi -computer $compname Win32_ComputerSystem | Format-Table @{Expression={$_.Username};Label="Current User"} >> D:\PC_MEM_Proc.txt
				"PC" >> D:\PC_MEM_Proc.txt
				gwmi -computer $compname Win32_ComputerSystem | Format-List Name,Manufacturer,Model,SystemType,BIOSVersion >> D:\PC_MEM_Proc.txt
				"OS" >> D:\PC_MEM_Proc.txt
				gwmi -computer $compname Win32_OperatingSystem | Format-List @{Expression={$_.Caption};Label="OS Name"},OSArchitecture >> D:\PC_MEM_Proc.txt
				"Processor :" >> D:\PC_MEM_Proc.txt
				gwmi -computer $compname Win32_Processor | Format-List Caption,Name,Manufacturer,NumberOfCores,AddressWidth >> D:\PC_MEM_Proc.txt
				$wmi = gwmi -computer $compname Win32_PhysicalMemory
				$memory_quantity = 0
				foreach($device in $wmi){ 
                #Write-Host "Bank Label:     " $device.BankLabel >> D:\$compname.txt
                #Write-Host "Capacity:       " ($device.Capacity/1MB) "Mb" >> D:\$compname.txt
                #Write-Host "Data Width:     " $device.DataWidth >> D:\$compname.txt
                #Write-Host "Device Locator: " $device.DeviceLocator >> D:\$compname.txt  
                #"" 
				$memory_quantity += ($device.Capacity/1MB)
				}
				"Total memory : " >> D:\PC_MEM_Proc.txt
				$memory_quantity >> D:\PC_MEM_Proc.txt
				"MB" >> D:\PC_MEM_Proc.txt
				Pause
			}
		 16 {
				$compname >> D:\hdd.txt
				Get-WmiObject win32_diskDrive -ComputerName $compname  | select Manufacturer, Model,InterfaceType,Size,Partitions >> D:\hdd.txt
			}
